...
sequelizeOwn.sync({ force: false, alter: false })
  .then(() => {
...
router.get("/new/sub-categoria/:categoriaName", async (req, res, next) => {
  const categoria = await models.Categoria.findOne({ where: { nombre: req.params.categoriaName}});
  const data = await models.SubCategoria.findAll({ where: { id_categoria: categoria.id}});
  response.success(req, res, data, 200)
})
...
router.get("/new/sub-categoria/:subId", async (req, res, next) => {
  const finalResponse = [];
  const productosList = await models.ProductoSubcategoria.findAll({ where: { id_categoria: req.params.subId}});
  
  await productosList.forEach(async el => {
    const product = await models.Producto.findAll({ where: { id: el.id_producto}});
